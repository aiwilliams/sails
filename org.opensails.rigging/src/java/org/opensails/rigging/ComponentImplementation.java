...
public class ComponentImplementation implements ComponentResolver {
	...
	protected Object instantiate() {
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
        	...
        	if (whenNotInstantiated != null && !container.resolver(parameterTypes[i]).isInstantiated())
				parameters[i] = container.instance(whenNotInstantiated.value(), whenNotInstantiated.value());
        	else
    ...
    protected boolean canSatisfy(Constructor constructor) {
		...
		for (int i = 0; i < parameterTypes.length; i++) {
			WhenNotInstantiated whenNotInstantiated = annotation(WhenNotInstantiated.class, constructor, i);
			if (!container.contains(parameterTypes[i]) && (whenNotInstantiated == null || findConstructor(whenNotInstantiated.value()) == null))
				canSatisfy = false;
		}
        return canSatisfy;
    }
