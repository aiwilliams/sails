...
public abstract class AbstractActionEventProcessor<C extends IEventProcessingContext> implements IActionEventProcessor {
	...
	protected C createInstanceOrNull(ISailsEvent event) {
		C instance = !hasImplementation() ? createNullInstance(event) : createInstance(event, processingContext);
		instance.setEventContext(event, this);
		return instance;
	}

	protected abstract C createNullInstance(ISailsEvent event);
}
